// دیپلوی بک‌اند: قابلیت «اعلان نسخه جدید اپ» (AppUpdate) — اجرای دستی توسط ادمین
//   node deploy/deploy-app-update.js
...
(async () => {
  try {
    console.log('=== DEPLOY APP-UPDATE (BACKEND) ===');

    await uploadFile('E:\\soha\\server\\models\\AppUpdate.js', '/opt/soha/server/models/AppUpdate.js');
    await uploadFile('E:\\soha\\server\\routes\\appUpdate.js', '/opt/soha/server/routes/appUpdate.js');
    ...
    await ssh('Restart backend', 'cd /opt/soha/server && npm install --omit=dev >/dev/null 2>&1; systemctl restart soha-backend && sleep 2 && systemctl is-active soha-backend', 60000);
    await ssh('Check health', 'curl -s http://localhost:5000/api/health', 10000);
    await ssh('Check app-update', 'curl -s http://localhost:5000/api/app-update/latest', 10000);
