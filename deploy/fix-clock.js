...
(async () => {
  try {
    console.log('=== FIX SERVER CLOCK ===');
    console.log('--- current time ---');
    console.log(await ssh('date; timedatectl 2>&1 | head -8'));
    console.log('--- enable NTP sync ---');
    console.log(await ssh('timedatectl set-ntp true 2>&1; sleep 8; date; timedatectl 2>&1 | grep -E "synchronized|NTP|Time zone"'));
    console.log('\n=== DONE ===');
  } catch (e) {
    console.error('\nFAILED:', e.message);
  }
