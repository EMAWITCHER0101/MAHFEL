...
(async () => {
  try {
    // Remove static alias, let Next.js handle everything
    ...
    console.log('HTML:', await ssh('curl -sI http://localhost:80/ 2>&1 | head -2'));
    console.log('JS:', await ssh('curl -sI "http://localhost:80/_next/static/chunks/$(curl -s http://localhost:3000/ | grep -oE \"[a-z0-9_-]+\\.js\" | head -1)" 2>&1 | head -3'));
