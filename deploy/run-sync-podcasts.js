...
(async () => {
  try {
    console.log('=== SYNC PODCASTS ===');
    console.log('Target: ' + HOST);

    console.log('\n--- 1. Upload script ---');
    await uploadFile(path.join(__dirname, 'sync-podcasts.mjs'), '/opt/soha/server/sync-podcasts.mjs');

    console.log('\n--- 2. Run sync ---');
    console.log(await ssh('cd /opt/soha/server && node sync-podcasts.mjs', 60000));

    console.log('\n--- 3. Cleanup + verify ---');
    console.log(await ssh('rm -f /opt/soha/server/sync-podcasts.mjs'));
    console.log(await ssh('curl -s http://localhost:3000/api/podcasts | head -c 300', 20000));
