...
(async () => {
  await ssh('IP sw.js', 'curl -s -o /dev/null -w "sw:%{http_code} (%{content_type})\\n" http://87.248.145.44/sw.js', 20000);
  await ssh('IP root', 'curl -s -o /dev/null -w "root:%{http_code}\\n" http://87.248.145.44/', 20000);
  await ssh('local 3000', 'curl -s -o /dev/null -w "l3000:%{http_code}\\n" http://localhost:3000/', 20000);
  await ssh('listen ports', 'netstat -tlnp 2>/dev/null | grep -E ":3000|:8080|:8081|:5000" | head -6', 20000);
})();