...
(async () => {
  try {
    await uploadFile(path.join(__dirname, 'list-missing-covers.mjs'), '/opt/soha/server/list-missing-covers.mjs');
    console.log(await ssh('cd /opt/soha/server && node list-missing-covers.mjs', 60000));
    console.log(await ssh('rm -f /opt/soha/server/list-missing-covers.mjs'));
  } catch (e) {
