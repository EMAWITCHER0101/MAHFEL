// دیپلوی بک‌اند: هزینه‌ها + آمار فروش — اجرای دستی
//   node deploy/deploy-expenses.js
...
(async () => {
  try {
    console.log('=== DEPLOY EXPENSES + SALES STATS (BACKEND) ===');
    await uploadFile('E:\\soha\\server\\models\\Expense.js', '/opt/soha/server/models/Expense.js');
    await uploadFile('E:\\soha\\server\\routes\\expenses.js', '/opt/soha/server/routes/expenses.js');
    await uploadFile('E:\\soha\\server\\server.js', '/opt/soha/server/server.js');
    await ssh('Restart backend', 'systemctl restart soha-backend && sleep 3 && systemctl is-active soha-backend', 60000);
