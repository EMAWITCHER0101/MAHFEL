// دیپلوی بک‌اند: فروش کتاب مجازی (PurchaseRequest) — اجرای دستی توسط ادمین
//   node deploy/deploy-purchases.js
...
(async () => {
  try {
    console.log('=== DEPLOY PURCHASE-REQUESTS (BACKEND) ===');

    await uploadFile('E:\\soha\\server\\models\\PurchaseRequest.js', '/opt/soha/server/models/PurchaseRequest.js');
    ...
    await ssh('Check route exists', "curl -s -o /dev/null -w '%{http_code}' http://localhost:5000/api/purchase-requests", 10000);
